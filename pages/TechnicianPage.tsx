...
// Samakan dengan backend form_statuses id kamu.
// Kalau backend kamu sudah punya constant ini di ../lib/api/admin, kamu boleh import dari sana.
...
async function updateBookingStatusOnServer(formId: number, status: BookingStatus, userId: number) {
  ...
  const res = await fetch(
    ...
    }/api/v1/admin/update-booking-status`,
    {
      method: 'PUT',
      credentials: 'include',
      ...
      body: JSON.stringify({
        form_id: formId,
        user_id: userId, // technician user_id
        status: statusCode,
      }),
    },
  );
...
const PhotoUpload: React.FC<{
  ...
  disabled?: boolean;
}> = ({ label, photoUrlOrPath, onUploadFile, disabled = false }) => {
  const [isUploading, setIsUploading] = useState(false);

  const resolved = useMemo(() => resolveAssetUrl(photoUrlOrPath), [photoUrlOrPath]);
...
const JobCard: React.FC<{
  ...
  const pushStatusToServer = async (nextStatus: BookingStatus, patch?: Partial<Booking>) => {
    // booking.formId mungkin tidak ada di type, tapi dari API kita butuh formId.
    // Di storage API versi sebelumnya, aku mapping id = apply_id/form_id.
    // Agar aman: kita butuh formId eksplisit. Kalau kamu sudah bawa formId di Booking, pakai itu.
    ...
    await updateBookingStatusOnServer(Number(formId), nextStatus, currentTechnicianId);

    // update local dulu biar UI responsif
    ...
    // optional: refresh dari server biar sinkron (foto/status/field lain)
    await onRefresh();
  };
  ...
  const executeCompleteJob = async () => {
    try {
      ...
      // update local + server status completed
          ...
          {(booking.arrivalTime || booking.startTime || booking.endTime) && (
            <div className="mt-3 pt-3 border-t border-gray-200 dark:border-slate-700/50">
              <h4 className="font-semibold text-gray-700 dark:text-gray-200 mb-1">
                Log Waktu Pengerjaan
              </h4>
              {booking.arrivalTime && (
                <p>
                  <strong>Tiba di Lokasi:</strong>{' '}
              ...
              {booking.workDurationMinutes != null && (
                <p>
                  <strong>Total Durasi Kerja:</strong> {booking.workDurationMinutes} menit
                </p>
              )}
            </div>
          )}
...
const TechnicianPage: React.FC = () => {
  ...
  const handleBookingUpdateLocal = (updatedBooking: Booking) => {
    // update state lokal cepat (optimistic UI)
  ...
  // BUTUH currentUser.id buat update status ke server (update-booking-status)
  const currentTechnicianId = (currentUser as any)?.id;
  if (!currentTechnicianId) {
    return (
      <div className="text-center py-20">
        currentUser.id tidak ada. Pastikan endpoint auth/me mengembalikan id user.
      </div>
    );
  }
          ...
          <div key="jobs">
            <div className="mb-6">
              ...
              <button
                ...
                className="ml-3 px-4 py-2 text-sm font-semibold rounded-md bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 hover:bg-gray-50 dark:hover:bg-slate-700"
              >
                Refresh
              </button>
            </div>
            ...
            {jobFilter === 'upcoming' &&
              (isUpcomingEmpty ? (
                <div
                  className="text-center py-20 bg-white dark:bg-slate-800 rounded-xl shadow-md"
                  data-aos="zoom-in"
                >
              ...
              {upcomingJobsFlat.length > 0 ? (
                upcomingJobsFlat.map((job) => (
                  ...
                  >
                    <button
                      onClick={() =>
                        setSelectedLocation({ lat: job.lat, lng: job.lng, title: job.name })
                      }
          ...
          <nav className="flex-grow p-4 space-y-2">
            {[
              ...
              <button
                key={item.section}
                onClick={() => {
                  setActiveSection(item.section as TechnicianSection);
