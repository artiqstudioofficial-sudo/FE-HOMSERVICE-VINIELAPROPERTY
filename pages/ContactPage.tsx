...
const ContactFormSection: React.FC = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const availability: any = await getAvailability();
        if (!mounted) return;
        ...
        setFullyBookedDates(new Set(Array.isArray(fully) ? fully : []));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setBookedSlots((prev) => {
        const next = { ...prev };
        if (Number.isFinite(usedFromServer) && usedFromServer >= 0) {
          next[slotKey] = usedFromServer;
        } else {
          next[slotKey] = (next[slotKey] || 0) + 1;
        }
        return next;
      });

      // (optional) kalau kamu memang mem-block semua tanggal saat durasi > 1 hari
